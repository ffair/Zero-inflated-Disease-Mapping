...
clusterEvalQ(clus,{
  library(MASS)
  library(pscl)})
list_in = list(c(-6.5,0.95),c(-6.5,0.1),c(-8.5,0.95),c(-8.5,0.1))
res_art = parLapply(clus, list_in, fun = simul_art_spatial)
...
#############################################################################
n1 = 4 #Number of simulation settings
...
set.seed(5)
for(ii in 1:n1){
  beta_true = as.matrix(c(list_in[[ii]][1],0.1,-0.2,0.3,-0.4),ncol=1)
  alpha_true = as.matrix(c(list_in[[ii]][2],0.5),ncol=1)
...
plot_beta(1)
ggsave("alpha=0.95_beta=-6.5.png",width=4,height=3)
plot_beta(2)
ggsave("alpha=0.1_beta=-6.5.png",width=4,height=3)
plot_beta(3)
ggsave("alpha=0.95_beta=-8.5.png",width=4,height=3)
plot_beta(4)
ggsave("alpha=0.1_beta=-8.5.png",width=4,height=3)
